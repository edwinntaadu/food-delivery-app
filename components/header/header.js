...
import { renderFavoriteMeals } from '../favoriteMeals/favoriteMeals.js';
import { renderBookmarkedRestaurants } from '../bookmarkedRestaurants/bookmarkedRestaurants.js';
...
let district_city = "Unknown District"; // Default value
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    // Attach event listener to the dynamically created search box with debouncing
    const searchInput = document.getElementById("home-search-box");
    if (searchInput) {
      searchInput.addEventListener("input", debounce(async function () {
        const searchTerm = searchInput.value.trim();
        const isLikePage = window.location.pathname.includes("like.html");
        const containerId = isLikePage ? "fav_bookmark_list-container" : "dynamic-body-container";
        const dynamicBodyContainer = document.getElementById(containerId);
        ...
        if (searchTerm.length > 0) {
          try {
            ...
            let results;
            if (isLikePage) {
              // Filter results based on disp_category
              const disp_category = window.disp_category || "food";
              results = disp_category === "food" ? result_1 : result_2;
            } else {
              // Combine results for home.html
            ...
            if (results.length > 0) {
              const foodCards = results.map(item => {
                if (item.category) {
                  // Render food card
          ...
          if (isLikePage) {
            const disp_category = window.disp_category || "food";
            dynamicBodyContainer.innerHTML = await (disp_category === "food" ? renderFavoriteMeals() : renderBookmarkedRestaurants());
          } else {
            dynamicBodyContainer.innerHTML = await renderHomeDefault();
          }
        }
